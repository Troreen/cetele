...
export function AccountSetup({ recovery = false, mentorship = true }: { recovery?: boolean; mentorship?: boolean }) {
  ...
  return <main className="auth-page"><section className="auth-card onboarding-card">
    <div className="brand"><span className="brand-mark"><Check size={15} /></span>Mülahaza</div>
    <div className="auth-copy"><h1>{recovery ? "Yeni parolanı belirle" : "Mülahaza kimliğini kur"}</h1><p>{recovery ? "Doğrulanmış kurtarma e-postanla açılan bu oturumda yeni parolanı kaydet." : "Kısa kurulumdan sonra kendi kaydına geçeceksin. Bu metinler yasal onay bekleyen non-production fixture kopyadır."}</p></div>
    ...
    <form className="form-stack" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      {!recovery ? <fieldset className="onboarding-section"><legend>Senin Mülahaza kimliğin</legend><label>Kullanıcı adı<input value={alias} onChange={(event) => setAlias(event.target.value)} minLength={2} maxLength={40} autoComplete="nickname" required /></label></fieldset> : null}
