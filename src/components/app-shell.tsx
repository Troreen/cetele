  ...
  return (
    <div className="app-frame">
      <header className="topbar">
        <Link href="/today" className="brand" aria-label="Çetele ana sayfa"><span className="brand-mark" aria-hidden="true">✓</span>Çetele</Link>
      ...
      <aside className="side-rail" aria-label="Ana menü">
        <nav className="side-nav">
          {visibleNavigation.map((item) => {
            ...
            return <Link key={item.href} href={item.href} className={clsx("nav-link", active && "active")} aria-current={active ? "page" : undefined}><Icon size={19} /><span>{item.label}</span>{count > 0 ? <strong className="nav-count" aria-label={`${count} açık kayıt`}>{count}</strong> : null}</Link>;
          })}
        </nav>
      ...
      <nav className="bottom-nav" aria-label="Mobil menü">
        {visibleNavigation.filter((item) => ["/today", "/progress", "/students"].includes(item.href)).map((item) => {
          const Icon = item.icon;
          const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
          return <Link key={item.href} href={item.href} className={clsx(active && "active")} aria-current={active ? "page" : undefined}><Icon size={21} /><span>{item.label}</span></Link>;
        })}
