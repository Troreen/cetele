...
import { assignmentsFor, currentWeekDates, person, recentDates } from "@/modules/cetele/selectors";
...
export function StudentDetailScreen({ studentId }: { studentId: string }) {
  ...
  if (!student || student.mentorId !== state.currentUserId) {
    return <div className="workspace"><h1>Öğrenci bulunamadı</h1><p>Yalnızca doğrudan öğrencilerinin kayıtlarına erişebilirsin.</p></div>;
  }
  ...
  return <div className="workspace detail-workspace">
    <Link href={hrefWithUiState("/students", uiSearch)} className="back-link"><ArrowLeft size={17} /> Öğrencilerim</Link>
    <header className="student-detail-header">
      <span className="avatar large">{student.initials}</span>
      <div><h1>{student.name}</h1><p>{student.invitation === "pending" ? "Davet kabulü bekleniyor" : `${assignments.length} etkin alışkanlık · Sorumlu mentor: ${responsibleMentor?.name ?? "—"}`}</p></div>
      <span className={open ? "status-text attention" : "status-text calm"}>{open ? "Dikkat gerekiyor" : "Sorun görünmüyor"}</span>
      <div className="section-actions"><button className="secondary-button" onClick={() => setExcuseOpen(true)}><CalendarOff size={17} /> Mazeretli gün</button><Link href={hrefWithUiState("/library", uiSearch)} className="primary-button"><Plus size={17} /> Alışkanlık ata</Link></div>
    </header>
    <div className="range-tabs detail-range" role="tablist" aria-label="Öğrenci alışkanlık tarih aralığı">
