...
import { Bell, Download, Moon, RotateCcw, ShieldCheck, Sun, Trash2 } from "lucide-react";
...
import { requestAccountDeletion, requestAccountExport, withdrawConsent } from "@/modules/cetele/account-actions";
...
export function SettingsScreen() {
  ...
  const runAccountAction = async (action: () => Promise<void>, confirmation?: string) => {
  ...
  return <div className="workspace settings-workspace"><SectionHeader title="Ayarlar" description="Hatırlatmalar, görünüm ve yerel demo verileri." />
    ...
    <section className="settings-section"><div className="settings-heading"><ShieldCheck size={20} /><div><h2>Gizlilik ve hesap</h2><p>Yasal kanıtlar, rıza tercihleri ve veri hakları.</p></div></div><div className="account-rights"><a className="secondary-button" href="/legal/privacy">Gizlilik Bildirimi</a><a className="secondary-button" href="/legal/terms">Hizmet Koşulları</a><button className="secondary-button" type="button" onClick={() => void runAccountAction(requestAccountExport)}><Download size={17} /> Verilerimin dışa aktarımını iste</button><a className="secondary-button" href="/account/export"><Download size={17} /> İstenen dışa aktarımı indir</a><button className="secondary-button" type="button" onClick={() => void runAccountAction(() => withdrawConsent({ purpose: "direct_mentor_visibility" }), "Doğrudan Mentor görünürlüğünü geri çekmek mentorun erişimini hemen durdurur. Devam edilsin mi?")}>Doğrudan Mentor rızasını geri çek</button><button className="secondary-button" type="button" onClick={() => void runAccountAction(() => withdrawConsent({ purpose: "core_tracking" }), "Çekirdek Takip Rızasını geri çekmek yeni kayıtları durdurur ve hesap kapatma yolunu başlatır. Devam edilsin mi?")}>Takip rızasını geri çek</button><button className="secondary-button danger" type="button" onClick={() => void runAccountAction(requestAccountDeletion, "Hesap silme isteği erişimi durdurur. Devam edilsin mi?")}><Trash2 size={17} /> Hesap silme isteği</button></div></section>
  </div>;
}
